...
async function install(){const installed=await installPwa();notice.value=installed?'Litera instalado como aplicativo.':'O navegador ainda não liberou a instalação neste dispositivo.'}
</script>
...
<template>
  <div>
    ...
    <template v-else>
      ...
      <section class="settings-section"><h2>Aplicativo</h2><p>Instale o Litera pelo fluxo oferecido pelo navegador para abrir em uma janela própria.</p><p v-if="pwaInstall.installed" class="alert alert--success">O Litera já está instalado neste dispositivo.</p><UiButton v-else variant="primary" :disabled="!pwaInstall.available" @click="install">{{ pwaInstall.available?'Instalar Litera':'Instalação indisponível' }}</UiButton><p v-if="!pwaInstall.available&&!pwaInstall.installed" class="field__hint">Quando compatível, o navegador libera a instalação após validar o acesso seguro e um breve uso do site.</p></section>
      <section class="settings-section"><h2>Segurança da conta</h2><form class="account-form" @submit.prevent="changePassword"><UiField label="Senha atual" for-id="current-password"><input id="current-password" v-model="currentPassword" type="password" autocomplete="current-password" required /></UiField><UiField label="Nova senha" for-id="new-password" hint="Use pelo menos 12 caracteres."><input id="new-password" v-model="newPassword" type="password" minlength="12" autocomplete="new-password" required /></UiField><UiButton type="submit">Alterar senha</UiButton></form></section>
    </template>
  </div>
</template>
