...
function toggleCollapsed(){collapsed.value=!collapsed.value;try{window.localStorage.setItem(`litera-sidebar-${auth.user?.id||'user'}`,String(collapsed.value))}catch{/* Optional layout preference. */}}
...
onMounted(async()=>{window.addEventListener('keydown',onKey);try{collapsed.value=window.localStorage.getItem(`litera-sidebar-${auth.user?.id||'user'}`)==='true'}catch{/* Optional layout preference. */}try{const result=await api<any>('/api/v1/settings');applyTheme(result.preferences.appTheme as AppTheme)}catch{/* Cached/system theme stays usable while offline. */}})
...
<template>
  <div class="app-shell" :class="{ 'app-shell--collapsed': collapsed }">
    ...
    <main id="main" class="page"><OfflineStatus /><RouterView :key="route.path" /></main>
  </div>
</template>
