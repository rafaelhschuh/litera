...
`.github/workflows/release.yml` roda para tags `v*.*.*`, exige que a tag corresponda à versão de `package.json`, repete todos os gates, publica as tags semânticas da imagem e só então cria a GitHub Release. Para `v0.4.2`, as tags são `0.4.2`, `0.4`, `0` e `latest`.

Processo de release:

```bash
git tag -a v0.4.2 -m "Litera 0.4.2"
